...
# Generate 10 values from a Binomial(8, 0.7) random variable
x = rbinom(10, 8, 0.7)
cat('rbinom(10, 8, 0.7):', x, '\n')  

# Generate 15 values from a Binomial(8, 0.2) random variable
x = rbinom(15, 8, 0.2)
cat('rbinom(15, 8, 0.2):', x, '\n')  

# A Bernoulli(p) random variable is just a Binomial with ntosses = 1.
# Generate 10 values from a Bernoulli(0.5) random variable
...
# For Y ~ Binomial(8, 0.6), compare with answer in exercise 1b.
#dbinom computes p(6) = P(Y = 6)
x = dbinom(6, 8, 0.6) # = 0.2090189
...
#Compute P(Y <= 6) for Y ~ Binomial(8, 0.6). 
...
# Generate 500 Binomial(8, 0.7) random values.
